...
export default function Contact () {
    return (
        <div className="px-4 flex flex-col space-y-4 md:flex-row md:space-x-4 " id="contact">
            {contactDetails.map((contacts, index) => {
                return (
                    <div key={index} className="mt-4 h-24 w-full bg-gold-200 rounded-xl shadow-xl border-copper-500 border-2">
                        <div className="mx-auto mt-2 mb-1 h-6 w-6">
                            <svg className="object-center fill-current text-copper-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                            {contacts.icon}
                            </svg>
                        </div>
